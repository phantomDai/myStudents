 ...
 * @date: 2018/4/28
 * @time: 9:55
 * @des: 变异体输出，主要内容与mujava.op.util.MutantCodeWriter一致
 *      主要针对部分编码规范问题进行修改
 */
...
public class ConcurrentMutantCodeWriter extends ParseTreeVisitor {
    ...
    public ConcurrentMutantCodeWriter(PrintWriter out) {
        super();
        this.out = out;
    }

    public ConcurrentMutantCodeWriter(String mutant_dir, PrintWriter out) {
        super();
        this.out = out;
    ...
    public void visit(CompilationUnit p)
            throws ParseTreeException {
        out.println("/* This is a concurrent mutant program generated by CT4Droid!");
        line_num++;
        out.println("* @author: GuYouda " + getTime());
        line_num++;
        out.println("*/");
        line_num++;
        ...
        if (islst.length != 0) {
            for (String anIslst : islst) {
                out.println("import " + anIslst + ";");
                line_num++;
            }
            out.println();
            line_num++;
    ...
    public String remove(String str, String target, int start_index) {
        int length = str.length();
        int offset = target.length();
        String result = str.substring(0, start_index);
        String last = str.substring(start_index + offset, length);
        result = result.concat(last);
        return result;
    }
