...
public class RunTestPanel extends JPanel implements ActionListener {
    ...
    JRadioButton onlyConcurrentButton = new JRadioButton("Execute only concurrent mutants");
    ...
    JPanel conResultPanel = new JPanel();
    final int CLASS = 1;
    ...
    void jbInit() {
        ...
        onlyConcurrentButton.setActionCommand("CONCURRENT");
        onlyConcurrentButton.addActionListener(this);
        ...
        group.add(onlyConcurrentButton);
        ...
        optionP.add(onlyConcurrentButton);
        ...
        JPanel concurrent_summaryPanel = new JPanel();
        concurrent_summaryPanel.setLayout(new BoxLayout(concurrent_summaryPanel, BoxLayout.PAGE_AXIS));

        JScrollPane conmTablePanel = new JScrollPane();
        CONMSummaryTableModel conmodel = new CONMSummaryTableModel();
        conmTable = new JTable(conmodel);
        adjustSummaryTableSize(conmTable, conmodel);
        conmTablePanel.getViewport().add(conmTable);
        conmTablePanel.setPreferredSize(new Dimension(120, 500));
        conmTablePanel.setMaximumSize(new Dimension(120, 500));

        concurrent_summaryPanel.add(conmTablePanel);
        concurrent_summaryPanel.add(conmTotalLabel);

        summaryPanel.add(traditional_summaryPanel);
        summaryPanel.add(class_summaryPanel);
        summaryPanel.add(concurrent_summaryPanel);
        ...
        JLabel label_time = new JLabel("TimeUtil-Out : ", JLabel.RIGHT);
...
//      
        ...
        conResultPanel.setBorder(new TitledBorder("Concurrent Mutants Result"));
        conResultPanel.setLayout(new GridBagLayout());
        GridBagConstraints conResultConstraints = new GridBagConstraints();
        ResultTableModel conResultTableModel = new ResultTableModel();
        conResultTable = new JTable(conResultTableModel);
        setResultTableSize(conResultTable);
        conResultConstraints.gridx = 0;
        conResultConstraints.gridy = 0;
        conResultConstraints.gridwidth = 2;
        conResultPanel.add(conResultTable, conResultConstraints);
        JScrollPane con_livePanel = new JScrollPane();
        setSPSize(con_livePanel);
        con_livePanel.setBorder(new TitledBorder("Live"));
        con_livePanel.getViewport().add(conLiveList);
        conResultConstraints.gridx = 0;
        conResultConstraints.gridy = 1;
        conResultConstraints.gridwidth = 1;
        conResultPanel.add(con_livePanel, conResultConstraints);
        JScrollPane con_killedPanel = new JScrollPane();
        setSPSize(con_killedPanel);
        con_killedPanel.setBorder(new TitledBorder("Killed"));
        con_killedPanel.getViewport().add(conKilledList);
        conResultConstraints.gridx = 1;
        conResultConstraints.gridy = 1;
        conResultPanel.add(con_killedPanel, conResultConstraints);
        resultPanel.add(conResultPanel);


        resultPanel.setPreferredSize(new Dimension(750, 520));
        resultPanel.setMaximumSize(new Dimension(1000, 520));
