...
public class MutantsGenPanel extends JPanel {
    ...
    JTable concurrentOpTable = new JTable();
    JButton concurrentNoneB = new JButton("None");
    JButton concurrentAllB = new JButton("All");
...
//   JButton descriptionB = new JButton("Mutation operators\' description");
    ...
    void jbInit() {
        ...
        fileSP.setPreferredSize(new Dimension(800, 500));
        ...
        // RIGHT part
        rightPanel.setPreferredSize(new Dimension(500, 880));
        rightPanel.setMaximumSize(new Dimension(500, 880));
...
//      rightPanel.add(descriptionBPanel);
        ...
        JPanel concurrent_operator_panel = new JPanel();
        concurrent_operator_panel.setBorder(new TitledBorder("Concurrence-level"));
        concurrent_operator_panel.setPreferredSize(new Dimension(140, 550));
        concurrent_operator_panel.setMaximumSize(new Dimension(140, 550));
        concurrent_operator_panel.setLayout(new BoxLayout(concurrent_operator_panel, BoxLayout.PAGE_AXIS));
        JScrollPane concurrent_operator_scrollP = new JScrollPane();
        concurrent_operator_panel.add(concurrent_operator_scrollP);
        // 变异算子表
        CONMOTableModel conTableModel = new CONMOTableModel();
        concurrentOpTable = new JTable(conTableModel);

        initColumnSizes(concurrentOpTable, conTableModel);
        concurrent_operator_scrollP.getViewport().add(concurrentOpTable, null);
        concurrent_operator_scrollP.setPreferredSize(new Dimension(115, 550));
        concurrent_operator_scrollP.setMaximumSize(new Dimension(115, 550));

        concurrent_operator_panel.add(concurrent_operator_scrollP);
        JPanel concurrentBPanel = new JPanel();
        concurrentBPanel.setLayout(new FlowLayout());
        concurrentBPanel.add(concurrentNoneB);
        concurrentNoneB.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                concurrentNoneB_mouseClicked(e);
            }
        });
        concurrentBPanel.add(concurrentAllB);
        concurrentAllB.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                concurrentAllB_mouseClicked(e);
            }
        });
        concurrent_operator_panel.add(concurrentBPanel);
        ...
        operator_panel.add(concurrent_operator_panel);
...
/*
    ...
    void runB_mouseClicked(MouseEvent e) {
        ...
        CONMOTableModel conmoTableModel = (CONMOTableModel) concurrentOpTable.getModel();
        String[] concurrent_ops = conmoTableModel.getSelectedOprators();

        if ((class_ops == null || class_ops.length == 0) &&
                (traditional_ops == null || traditional_ops.length == 0) &&
                (concurrent_ops == null || concurrent_ops.length == 0)) {
...
//            }
            ...
            genEngine.compileMutants();*/
                ...
                //do not generate concurrent mutants if no class concurrent operator is selected
                if (concurrent_ops != null) {
                    ConcurrentMutantsGenerator conmGenEngine;
                    //System.out.println("original_file: " + original_file);
                    //System.out.println("traditional_ops: " + traditional_ops);
                    conmGenEngine = new ConcurrentMutantsGenerator(original_file, concurrent_ops);
                    conmGenEngine.makeMutants();
                    conmGenEngine.compileMutants();
                }
    ...
    void deleteDirectory() {
        ...
        File conmDir = new File(MutationSystem.MUTANT_HOME + "/" + MutationSystem.DIR_NAME
                + "/" + MutationSystem.CONM_DIR_NAME);
        while (conmDir.delete()) {    // do nothing?
        }
    ...
    void setMutationSystemPathFor(String file_name) {
        try {
            ...
            String concurrent_mutant_dir_path = mutant_dir_path + "/" + MutationSystem.CONM_DIR_NAME;
            File concurrent_mutant_path = new File(concurrent_mutant_dir_path);
            concurrent_mutant_path.mkdir();
            ...
            MutationSystem.CONCURRENT_MUTANT_PATH = concurrent_mutant_dir_path;
     ...
     * @param e 鼠标事件
     */
    void concurrentAllB_mouseClicked(MouseEvent e) {
        CONMOTableModel table = (CONMOTableModel) concurrentOpTable.getModel();
        table.setAllSelectValue(true);
        concurrentOpTable.setModel(table);
        concurrentOpTable.repaint();
    }
     ...
     * @param e 鼠标事件
     */
    void concurrentNoneB_mouseClicked(MouseEvent e) {
        CONMOTableModel table = (CONMOTableModel) concurrentOpTable.getModel();
        table.setAllSelectValue(false);
        concurrentOpTable.setModel(table);
        concurrentOpTable.repaint();
 ...
 * add : concurrent-level mutation operators, 2018-03-22 GuYouda </p>
...
abstract class MOTableModel extends AbstractTableModel {
    ...
    public MOTableModel() {
        if (getOperatorType() == CMO) {
            ...
            op = MutationSystem.conm_operators;
        }
...
class CONMOTableModel extends MOTableModel {
