...
public class AllMutantsGenerator extends MutantsGenerator {
    ...
    public AllMutantsGenerator(File f, String[] cOP, String[] tOP, String[] conOP) {
        super(f);
        classOp = cOP;
        traditionalOp = tOP;
        concurrentOp = conOP;
    ...
    void genMutants() {
        ...
        if (concurrentOp != null && concurrentOp.length > 0) {
            Debug.println("* Generating concurrent mutants");
    ...
    void genConcurrentMutants(ClassDeclarationList cdecls) {
        for (int j = 0; j < cdecls.size(); ++j) {
            ClassDeclaration cdecl = cdecls.get(j);
    ...
    public void compileMutants() {
        ...
        if (concurrentOp != null && concurrentOp.length > 0) {
            Debug.println("* Compiling concurrent mutants into bytecode");
            MutationSystem.MUTANT_PATH = MutationSystem.CONCURRENT_MUTANT_PATH;
            super.compileMutants();
        }
    }
